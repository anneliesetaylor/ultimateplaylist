...
if (isset($_POST['title']) && isset($_POST['songartist'])) {

    // sanitizeMySQL() is a custom function, written below
    $title = sanitizeMySQL($conn, $_POST['title']);
    $songartist = sanitizeMySQL($conn, $_POST['songartist']);
    $genre = sanitizeMySQL($conn, $_POST['genre']);
    $users_name = sanitizeMySQL($conn, $_POST['users_name']);
    $explanation = sanitizeMySQL($conn, $_POST['explanation']);

    // the prepared statement - note: 6 question marks represent
    // 6 variables we will send to database separately
    $query = "INSERT INTO 2010playlist (songtitle, artist, genre, submittedby, explanation)
    VALUES (?, ?, ?, ?, ?)";
